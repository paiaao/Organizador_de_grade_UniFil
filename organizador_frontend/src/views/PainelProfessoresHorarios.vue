          ...
          <div class="grid grid-cols-5 gap-3 mb-3">
            ...
            >
              <div v-if="slot.horario" class="text-xs text-center text-stone-700 font-medium leading-relaxed">
                ...
                <div class="text-stone-500">{{ slot.horario.turma }}</div>
...
export default {
  ...
  async mounted() {
    ...
    await this.carregarHorarios(professorId);
  },
  methods: {
    ...
    async carregarHorarios(professorId) {
      try {
        const horariosRes = await this.api.get(`/api/horarios/por-professor/${professorId}`);
        ...
        for (const h of horarios) {
          if (h.posicaoSemana_id !== null) {
            ...
            const turmaNome = h.turma?.nomeTurma || 'Turma não informada';
    ...
    async exportarPDF() {
      ...
      const opt = {
        ...
        filename: `horarios_professor_${new Date().toISOString().slice(0, 10)}.pdf`,
