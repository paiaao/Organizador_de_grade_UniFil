  ...
  <div class="flex h-screen">
    ...
    <div class="flex-1 flex flex-col items-center justify-center bg-white px-8">
      <div class="w-full max-w-sm">
        ...
        <p class="text-stone-500 text-sm mb-8">Acesse sua conta de aluno</p>
...
const submitLogin = async () => {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      switch(response.data.usuario.permissao) {
        case 1:
        ...
        case 3:
          ...
          setTimeout(() => {
            router.push('/PainelAlunosHorarios');
          }, 1000);
          break;
        default:
          router.push('/');
    ...
    if (error.response?.status === 401) {
      mostrarMensagem('Matrícula ou senha incorretos', 'erro');
