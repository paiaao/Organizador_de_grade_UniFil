    ...
    <div class="w-64 bg-[#ff9421] flex flex-col">
      ...
      <nav class="flex-1 px-4 space-y-4 mt-2">
        <button v-for="item in menuItems":key="item.id"
          @click="navegar(item)":class="['w-full px-4 py-3 text-left text-white font-medium rounded-lg text-sm transition-all',
      ...
      <div class="bg-[#ffeccf] rounded-xl p-8 shadow-sm">
        <div class="grid grid-cols-6 gap-5 mb-5">
          <div
            ...
            :key="dia"
            class="bg-[#ffdead] p-3 rounded-lg text-center font-bold text-sm text-gray-800"
          >
            {{ dia }}
          </div>
        </div>
